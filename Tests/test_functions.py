...
def read_image_teser(opt):
    x = img.imread('%s' % (opt.input_name))
    x = np2torch_tester(x,opt)
    x = x[:,0:3,:,:]
    return x
...
def torch2uint8_tester(x):
    x = x[0,:,:,:]
    x = x.permute((1,2,0))
    x = 255*denorm_tester(x)
    x = x.cpu().numpy()
    x = x.astype(np.uint8)
    return x
...
def create_reals_pyramid_tester(real, opt):
    reals = []
...
def dilate_mask(mask,opt):
    ...
    mask = torch2uint8_tester(mask)
    mask = mask[:,:,0]
    mask = morphology.binary_dilation(mask,selem=element)
    mask = filters.gaussian(mask, sigma=5)
    nc_im = opt.nc_im
    opt.nc_im = 1
    mask = np2torch_tester(mask,opt)
    opt.nc_im = nc_im
